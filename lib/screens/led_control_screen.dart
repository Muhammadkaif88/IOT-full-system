...
class _LedControlScreenState extends State<LedControlScreen> {
  ...
  final _ipCtrl = TextEditingController(text: '192.168.1.19');
  ...
  void _sendCommand() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(ok ? '✓ LED updated!' : '❌ ESP32 connect alla'),
  ...
  void _showIpDialog() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('ESP32 IP Address', style: TextStyle(color: Colors.white, fontSize: 15)),
        content: Column(
          ...
          children: [
            Container(
              ...
              child: const Row(children: [
                Icon(Icons.info_outline, color: Color(0xFF5DCAA5), size: 16),
                SizedBox(width: 8),
                Expanded(child: Text('Serial Monitor il kaanunna IP:\n"ESP32 IP: 192.168.x.x"', style: TextStyle(color: Color(0xFF5DCAA5), fontSize: 11))),
              ]),
            ),
            const SizedBox(height: 14),
            const Text('ESP32 IP:', style: TextStyle(color: Colors.white60, fontSize: 12)),
            ...
            const SizedBox(height: 6),
            Text('Current: ${led.espIp}', style: const TextStyle(color: Colors.white38, fontSize: 11)),
          ],
        ),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          // Settings icon — always visible — tap to set IP
          IconButton(
              ...
              child: Row(children: [
                if (_connecting)
                  ...
                  Container(width: 8, height: 8, decoration: BoxDecoration(shape: BoxShape.circle,
                      color: _connected ? const Color(0xFF1D9E75) : const Color(0xFFE24B4A))),
                const SizedBox(width: 4),
                Text(
                  _connecting ? '...' : (_connected ? 'Live' : 'Offline'),
                  style: TextStyle(fontSize: 11,
                      color: _connecting ? const Color(0xFFEF9F27) : (_connected ? const Color(0xFF5DCAA5) : const Color(0xFFE24B4A))),
                ),
                const SizedBox(width: 4),
              ]),
            ),
          ...
          Switch(
            ...
            onChanged: (v) async {
              ...
              await led.sendFull(on: v, color: '#FF4500', effect: effects[selectedEffect]['name'] as String, brightness: brightness.round(), speed: speed.round());
            },
          ),
          const SizedBox(width: 4),
        ],
      ...
      body: ListView(
        ...
        children: [
          // IP setup banner — if not connected
              ...
              child: Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('ESP32 connect alla', style: TextStyle(color: Color(0xFFE24B4A), fontSize: 12, fontWeight: FontWeight.w500)),
                      Text('Current IP: ${led.espIp} — tap to change', style: const TextStyle(color: Color(0xFFE24B4A), fontSize: 10)),
                    ],
                  )),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                    decoration: BoxDecoration(color: const Color(0xFFE24B4A).withOpacity(0.2), borderRadius: BorderRadius.circular(8)),
                    child: const Text('Set IP', style: TextStyle(color: Color(0xFFE24B4A), fontSize: 11, fontWeight: FontWeight.w500)),
                  ),
                ]),
              ),
            ),
          ...
          _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              Container(
                ...
                child: Text(d.isOn ? 'On' : 'Off',
                    style: TextStyle(color: d.isOn ? const Color(0xFF5DCAA5) : Colors.white38, fontSize: 10)),
              ),
            ]),
          ])),
          ...
          _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: const BoxDecoration(
                    ...
                    gradient: SweepGradient(colors: [Color(0xFFFF0000),Color(0xFFFFFF00),Color(0xFF00FF00),Color(0xFF00FFFF),Color(0xFF0000FF),Color(0xFFFF00FF),Color(0xFFFF0000)]),
                  ),
                  child: Center(child: Container(width: 28, height: 28,
                      decoration: BoxDecoration(shape: BoxShape.circle, color: selectedColor, border: Border.all(color: Colors.white, width: 2)))),
                ),
              ),
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                ...
                const SizedBox(height: 6),
                Row(children: [
                  _rgb('R', selectedColor.red), const SizedBox(width: 4),
                  _rgb('G', selectedColor.green), const SizedBox(width: 4),
                  _rgb('B', selectedColor.blue),
                ]),
              ])),
            ]),
            const SizedBox(height: 12),
                  ...
                  child: Container(
                    ...
                    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      Text(e['label'] as String,
                          style: TextStyle(fontSize: 9, color: active ? Colors.white70 : Colors.white38)),
                    ]),
                  ),
                );
              },
      ...
      bottomNavigationBar: Padding(
        ...
        child: ElevatedButton.icon(
          ...
          style: ElevatedButton.styleFrom(
            backgroundColor: _connected ? const Color(0xFF6C63FF) : const Color(0xFF374151),
  ...
  Widget _lbl(String t) => Padding(
    ...
    child: Text(t, style: const TextStyle(color: Colors.white38, fontSize: 10, letterSpacing: 1, fontWeight: FontWeight.w500)),
  );
  Widget _rgb(String l, int v) => Expanded(
    child: Container(
      ...
      child: Column(children: [
        ...
        Text(l, style: const TextStyle(color: Colors.white38, fontSize: 8)),
      ]),
    ),
  );
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: ColorPicker(pickerColor: selectedColor, onColorChanged: (c) => setState(() => selectedColor = c), enableAlpha: false, portraitOnly: true),
        ),
        actions: [
          TextButton(onPressed: () { Navigator.pop(context); _sendCommand(); }, child: const Text('Done')),
        ],
      ),
    );
  }
